...
<html lang="en">
    <head>
        ...
        <script type = "text/javascript" src = "indexSearch.js"></script> 
        <script type = "text/javascript" src = "indexWatchlist.js"></script>
        <script type = "text/javascript" src = "indexSorting.js"></script>
    ...
    <body>
        ...
        <fieldset style = "float: right; border:none;">
            <table>
                <tr>
                    <td>Sort By : </td>
                    <td><select id = "myList">
                    <option onclick = 'sorting("Movie_Name")'>Title</option>
                    <option onclick = 'sorting("Rating")'>Rating</option>
                    <option onclick = 'sorting("Year")'>Year</option>
                    <option onclick = 'sorting("Genre")'>Genre</option>
                    </select></td>
                </tr>
            </table>
       </fieldset>
        <form id = "searchBox" method="post" action = "searchResult.php" enctype = "multipart/form-data">
            <table>
                <tr><td><label id="srch_msg" class="err_msg"></label></td></tr>
                <tr><td><input type="text" name="search" placeholder="search anything" size="28" onkeyup = "searchResult(this.value)"/></td>
                <td><input type="submit" value="Search" action = "index.php"/></td></tr>
                <tr><td><div id="livesearch"></div></td></tr>
            </table>
        </form>
        ...
        <div class="grid-container" id = "mainGridContainer">
            <?php
            $result = mysqli_query($mysqli,"SELECT * FROM Movie_Table ORDER BY Rating LIMIT 15");
            $MID;
            while($row = mysqli_fetch_array($result)){
                ...
                <div class="grid-item">
                    ...
                    <table style = "float: right;">
                        <tr><td><button onclick="watchlistDropdown(<?=$MID?>)">Add to Watchlist</button></td></tr>
                        <tr><td><div id="dropdown<?=$MID?>"></div></td></tr>
                    </table>
                </div>
            <?}?>
        </div>
