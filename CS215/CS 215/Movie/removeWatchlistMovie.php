<?php
    ...
    if(!empty($_POST)){
        ...
        $checkQuery = "SELECT * FROM Movie_Table WHERE Movie_Name = '$MName'";
        ...
        if($result->num_rows > 0){
            ...
            $query = "SELECT * FROM Bridge WHERE Movie_ID = '$MID'";
            $result = mysqli_query($mysqli,$query);
            if($result->num_rows > 0){
                $query = "DELETE FROM Bridge WHERE Watchlist_ID = '$watchlistID' AND Movie_ID = '$MID'";
                $result = runQuery($query);
                header("Location: watchlistLayout.php?watchlistID=$watchlistID");
                exit();
            }
            else{
                $error = "Movie Not in the Playlist";
            }
        }
        else{
...
<!DOCTYPE html>
    <html>
        ...
        <body>
            ...
            <!------------------------------------------------------------------------------------------------------------>
            <div class="container">
                <form id = "playlistForm" method="post" action="removeWatchlistMovie.php?watchlistID=<?=$watchlistID?>">
                    <div style = "color:red"> <?=$error?></div>
                    <table>
                        <tr><td></td><td><label id="plist_msg" class="err_msg"></label></td></tr>
                        <tr><td>Movie Name: </td><td> <input type="text" name="movie" size="28" onkeyup="charCount(this.value)"/></td></tr>
