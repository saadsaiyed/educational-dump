...
void chooseMove(int old_monster_row, int old_monster_column, int player_row, int player_column,	int &new_monster_row, int &new_monster_column, string &move_string);
...
int main()
{
	...
	while (monster_distance > 0.001)
	{
		...
		cout << "Monster" << " health" << player.health << "movecost" << player.total_move_cost << " score " << player.score << "  next: " << action << endl;
		...
		monster_distance = calculateDistance(monster_row, monster_column, player_row, player_column);
	...
	printLevel(level, monster_row, monster_column, player_row, player_column);
